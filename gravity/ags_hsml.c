...
int ags_gravity_kernel_shared_BITFLAG(short int particle_type_primary)
{
    ...
    if (!((1 << particle_type_primary) & (ADAPTIVE_GRAVSOFT_FORALL)))
    {
        return 0;
    } /* particle is NOT one of the designated 'adaptive' types */
#endif

#ifdef ADAPTIVE_GRAVSOFT_FROM_TIDAL_CRITERION
    if (!((1 << particle_type_primary) & (ADAPTIVE_GRAVSOFT_FROM_TIDAL_CRITERION)))
    {
    ...
    {
        return 1;
    } /* black hole particles are AGS-active, but using BH physics, they see only gas */
#endif
...
#ifdef DM_SIDM
    if ((1 << particle_type_primary) & (DM_SIDM))
    {
        return DM_SIDM;
    } /* SIDM particles see other SIDM particles, regardless of type/mass */
#endif

#ifdef DM_DMB
    if (particle_type_primary == 1)
    {
        return 2;
    } /* 2^1: DM particles see each other */
#endif
...
void ags_out2particle_density(struct OUTPUT_STRUCT_NAME *out, int i, int mode, int loop_iteration)
{
    ...
    if (out->AGS_vsig > PPP[i].AGS_vsig)
    {
        PPP[i].AGS_vsig = out->AGS_vsig;
    }
...
#if defined(AGS_FACE_CALCULATION_IS_ACTIVE)
    {
        ...
        for (k = 0; k < 3; k++)
        {
            for (j = 0; j < 3; j++)
            {
                ASSIGN_ADD(P[i].NV_T[k][j], out->NV_T[k][j], mode);
            }
        }
    }
#endif
    ...
    for (k = 0; k < 3; k++)
    {
        ASSIGN_ADD(P[i].AGS_VelMean[k], out->VelMean[k], mode);
    }
            ...
            for (n = 0; n < numngb_inbox; n++)
            {
                ...
                if (r2 < h2)
                {
                    ...
                    if (kernel.r > 0)
                    {
                        ...
                        {
                            v_dot_r *= 0.333333;
                        } // receding elements don't signal strong change in forces in the same manner as approaching/converging particles
                        double vsig = 0.5 * fabs(fac_mu * v_dot_r / kernel.r);
                        short int TimeBin_j = P[j].TimeBin;
                        if (TimeBin_j < 0)
                        {
                            TimeBin_j = -TimeBin_j - 1;
                        } // need to make sure we correct for the fact that TimeBin is used as a 'switch' here to determine if a particle is active for iteration, otherwise this gives nonsense!
                        if (vsig > out.AGS_vsig)
                        {
                            out.AGS_vsig = vsig;
                        }
                        ...
                        if (!(TimeBinActive[TimeBin_j]) && (All.Time > All.TimeBegin) && (vsig > WAKEUP * P[j].AGS_vsig))
                        {
                            wakeup_condition = 1;
                        }
#if defined(GALSF)
                        if ((P[j].Type == 4) || ((All.ComovingIntegrationOn == 0) && ((P[j].Type == 2) || (P[j].Type == 3))))
                        {
                            wakeup_condition = 0;
                        } // don't wakeup star particles, or risk 2x-counting feedback events! //
#endif
...
void ags_density(void)
{
    ...
    double fac, fac_lim, desnumngb, desnumngbdev;
    long long ntot;
...
#include "../system/code_block_xchange_perform_ops.h" /* this calls the large block of code which actually contains all the loops, MPI/OPENMP/Pthreads parallelization */
        ...
        for (i = FirstActiveParticle, npleft = 0; i >= 0; i = NextActiveParticle[i])
        {
            if (ags_density_isactive(i))
            {
                ...
                if (ConditionNumber > MAX_REAL_NUMBER)
                {
                    PRINT_WARNING("CNUM for CBE: ThisTask=%d i=%d ConditionNumber=%g desnumngb=%g NumNgb=%g iter=%d NVT=%g/%g/%g/%g/%g/%g AGS_Hsml=%g \n", ThisTask, i, ConditionNumber, desnumngb, PPP[i].NumNgb, iter, P[i].NV_T[0][0], P[i].NV_T[1][1], P[i].NV_T[2][2], P[i].NV_T[0][1], P[i].NV_T[0][2], P[i].NV_T[1][2], PPP[i].AGS_Hsml);
                }
                if (iter > 10)
                {
                    desnumngbdev = DMIN(0.25 * desnumngb, desnumngbdev * exp(0.1 * log(desnumngb / (16. * desnumngbdev)) * ((double)iter - 9.)));
                }
#else
                if (iter > 4)
                {
                    desnumngbdev = DMIN(0.25 * desnumngb, desnumngbdev * exp(0.1 * log(desnumngb / (16. * desnumngbdev)) * ((double)iter - 3.)));
                }
#endif
                if (All.Time <= All.TimeBegin)
                {
                    if (desnumngbdev > 0.0005)
                        desnumngbdev = 0.0005;
                    if (iter > 50)
                    {
                        desnumngbdev = DMIN(0.25 * desnumngb, desnumngbdev * exp(0.1 * log(desnumngb / (16. * desnumngbdev)) * ((double)iter - 49.)));
                    }
                }
                ...
                if (redo_particle)
                {
                    ...
                    if ((particle_set_to_maxhsml_flag == 0) && (particle_set_to_minhsml_flag == 0))
                    {
                        ...
                        if (Right[i] > 0 && Left[i] > 0)
                        {
                            ...
                            if (iter > 1)
                            {
                                maxjump = 0.2 * log(Right[i] / Left[i]);
                            }
                            if (PPP[i].NumNgb > 1)
                            {
                            ...
                            if ((PPP[i].AGS_Hsml < Right[i]) && (PPP[i].AGS_Hsml > Left[i]))
                            {
                                if (iter > 1)
                                {
                                    ...
                                    if (PPP[i].AGS_Hsml > Right[i] / hfac)
                                    {
                                        PPP[i].AGS_Hsml = Right[i] / hfac;
                                    }
                                    if (PPP[i].AGS_Hsml < Left[i] * hfac)
                                    {
                                        PPP[i].AGS_Hsml = Left[i] * hfac;
                                    }
                                }
                            }
                            else
                            {
                                if (PPP[i].AGS_Hsml > Right[i])
                                    PPP[i].AGS_Hsml = Right[i];
                                if (PPP[i].AGS_Hsml < Left[i])
                                    PPP[i].AGS_Hsml = Left[i];
                        ...
                        else
                        {
                            ...
                            if (Right[i] == 0 && Left[i] > 0)
                            {
                                ...
                                if ((PPP[i].NumNgb < 2 * desnumngb) && (PPP[i].NumNgb > 0.1 * desnumngb))
                                {
                                    ...
                                    if (iter >= 4)
                                        if (PPP[i].DhsmlNgbFactor == 1)
                                            fac *= 10; // tries to help with being trapped in small steps
                            ...
                            if (Right[i] > 0 && Left[i] == 0)
                            {
                                ...
                                if (fac_lim < -1.535)
                                    fac_lim = -1.535; // decreasing N_ngb by factor ~100
                    ...
                    if (PPP[i].AGS_Hsml > maxsoft)
                        PPP[i].AGS_Hsml = maxsoft;
                    if (particle_set_to_maxhsml_flag == 1)
                        PPP[i].AGS_Hsml = maxsoft;
...
#include "../system/code_block_xchange_perform_ops_demalloc.h" /* this de-allocates the memory for the MPI/OPENMP/Pthreads parallelization block which must appear above */
    ...
    for (i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if (ags_density_isactive(i))
        {
            if ((P[i].Mass > 0) && (PPP[i].AGS_Hsml > 0) && (PPP[i].NumNgb > 0))
            {
                ...
                double Prho = 0 * h_eff * h_eff / 2.;
                if (P[i].Particle_DivVel > 0)
                {
                    Prho = -Prho;
                } // truelove criterion. NJeans[above] , gamma=2 for effective EOS when this dominates, rho=ma*na; h_eff here can be Hsml [P/rho~H^-1] or gravsoft_min to really enforce that, as MIN, with P/rho~H^-3; if-check makes it so this term always adds KE to the system, pumping it up
                ...
                else
                {
                    ...
                    for (k = 0; k < 3; k++)
                        P[i].AGS_VelMean[k] /= P[i].AGS_NgbInt;
                    ...
                    double new_vel_disp = (vel_disp - meanv_mag2) / All.cf_atime / All.cf_atime;
                    // double new_vel_disp = (1./All.cf_atime) * sqrt(vel_disp - meanv_mag2);

                    // if negative due to floating point error, set to zero
                    if (new_vel_disp <= 0 && fabs((vel_disp - meanv_mag2) / vel_disp) < 1e-5)
                        new_vel_disp = 0.0;

                    // if something went wrong (div by zero) or vel disp is still negative, warn
                    if (isnan(new_vel_disp) || new_vel_disp < 0)
                    {
                        printf("AGS_VelDisp is negative or NaN\n");
                        printf("  veldisp = %.3e\n", vel_disp);
    ...
    timeall = timediff(t00_truestart, t1);
...
int ags_density_isactive(int i)
{
    ...
    {
        default_to_return = 0;
    } /* check our 'marker' for particles which have finished iterating to an Hsml solution (if they have, dont do them again) */
...
double ags_return_maxsoft(int i)
{
    ...
    if (P[i].Type == 5)
    {
        maxsoft = All.BlackHoleMaxAccretionRadius / All.cf_atime;
    } // MaxAccretionRadius is now defined in params.txt in PHYSICAL units
#endif
    return maxsoft;
}
...
double do_cbe_nvt_inversion_for_faces(int i)
{
    ...
    MyLongDouble NV_T[3][3], Tinv[3][3];
    int j, k;
    for (j = 0; j < 3; j++)
    {
        for (k = 0; k < 3; k++)
        {
            NV_T[j][k] = P[i].NV_T[j][k];
        }
    }
    ...
    for (j = 0; j < 3; j++)
    {
        for (k = 0; k < 3; k++)
        {
            NV_T[j][k] /= dimensional_NV_T_normalizer;
        }
    } /* now NV_T should be dimensionless */
    ...
    while (1)
    {
        ...
        if (ConditionNumber < ConditionNumber_threshold)
        {
            break;
        } // end loop if we have reached target conditioning for the matrix
        for (j = 0; j < NUMDIMS; j++)
        {
            NV_T[j][j] += conditioning_term_to_add;
        } /* add the conditioning term which should make the matrix better-conditioned for subsequent use: this is a normalization times the identity matrix in the relevant number of dimensions */
    ...
    for (j = 0; j < 3; j++)
    {
        for (k = 0; k < 3; k++)
        {
            P[i].NV_T[j][k] = Tinv[j][k] / dimensional_NV_T_normalizer;
        }
    } // now P[i].NV_T holds the inverted matrix elements //
#ifdef CBE_DEBUG
    if ((ThisTask == 0) && (ConditionNumber > 1.0e10))
    {
        printf("Condition number == %g (Task=%d i=%d)\n", ConditionNumber, ThisTask, i);
    }
#endif
...
static inline void INPUTFUNCTION_NAME(struct INPUT_STRUCT_NAME *in, int i, int loop_iteration)
{
    ...
    for (k = 0; k < 3; k++)
    {
        in->AGS_Gradients_Density[k] = P[i].AGS_Gradients_Density[k];
    }
    for (k = 0; k < 3; k++)
    {
        for (k2 = 0; k2 < 3; k2++)
        {
            in->AGS_Gradients2_Density[k][k2] = P[i].AGS_Gradients2_Density[k][k2];
        }
    }
    ...
    for (k = 0; k < 3; k++)
    {
        in->AGS_Gradients_Psi_Re[k] = P[i].AGS_Gradients_Psi_Re[k];
    }
    for (k = 0; k < 3; k++)
    {
        for (k2 = 0; k2 < 3; k2++)
        {
            in->AGS_Gradients2_Psi_Re[k][k2] = P[i].AGS_Gradients2_Psi_Re[k][k2];
        }
    }
    in->AGS_Psi_Im = P[i].AGS_Psi_Im_Pred * P[i].AGS_Density / P[i].Mass;
    for (k = 0; k < 3; k++)
    {
        in->AGS_Gradients_Psi_Im[k] = P[i].AGS_Gradients_Psi_Im[k];
    }
    for (k = 0; k < 3; k++)
    {
        for (k2 = 0; k2 < 3; k2++)
        {
            in->AGS_Gradients2_Psi_Im[k][k2] = P[i].AGS_Gradients2_Psi_Im[k][k2];
        }
    }
#endif
#endif
...
static inline void OUTPUTFUNCTION_NAME(struct OUTPUT_STRUCT_NAME *out, int i, int mode, int loop_iteration)
{
    ...
    for (k = 0; k < 3; k++)
    {
        P[i].GravAccel[k] += out->acc[k];
    } // currently incompatible with hermite integrator -- need to update to Other_Accel
...
{
    if (P[i].TimeBin < 0)
        return 0; /* check our 'marker' for particles which have finished iterating to an Hsml solution (if they have, dont do them again) */
#ifdef DM_SIDM
    if ((1 << P[i].Type) & (DM_SIDM))
        return 1;
#endif
...
int AGSForce_evaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex, int *ngblist, int loop_iteration)
{
    ...
    struct OUTPUT_STRUCT_NAME out;
    memset(&out, 0, sizeof(struct OUTPUT_STRUCT_NAME));
    memset(&kernel, 0, sizeof(struct kernel_AGSForce));
    if (mode == 0)
    {
    ...
    if (local.Mass <= 0 || local.AGS_Hsml <= 0)
        return 0;
    ...
    while (startnode >= 0)
    {
        while (startnode >= 0)
        {
            ...
            {
                j = ngblist[n]; /* since we use the -threaded- version above of ngb-finding, its super-important this is the lower-case ngblist here! */
                if ((P[j].Mass <= 0) || (PPP[j].AGS_Hsml <= 0))
                    continue; /* make sure neighbor is valid */
...
#if defined(DM_SIDM)
                if (kernel.r > kernel.h_i + kernel.h_j)
                    continue;
#else
                if (kernel.r > kernel.h_i && kernel.r > kernel.h_j)
                    continue;
#endif
...
#include "../sidm/sidm_core_flux_computation.h"
    ...
    if (mode == 0)
    {
        OUTPUTFUNCTION_NAME(&out, target, 0, loop_iteration);
    }
    else
    {
        DATARESULT_NAME[target] = out;
    } /* collects the result at the right place */
    return 0;
}
