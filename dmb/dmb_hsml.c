...
void dmb_out2particle(struct OUTPUT_STRUCT_NAME *out, int i, int mode, int loop_iteration)
{
    ...
    for (k = 0; k < 3; k++)
    {
        ASSIGN_ADD(P[i].DMB_V[k], out->V[k], mode);
    }
...
void dmb_calc(void)
{
    ...
    MyFloat *Left, *Right;
    double desnumngb = 64, desnumngbdev = 48;
    long long ntot;
...
#include "../system/code_block_xchange_perform_ops.h"
        ...
        for (i = FirstActiveParticle, npleft = 0; i >= 0; i = NextActiveParticle[i])
        {
            if (dmb_isactive(i))
            {
                ...
                if (((P[i].DMB_NumNgb < desnumngb - desnumngbdev) || (P[i].DMB_NumNgb > (desnumngb + desnumngbdev))) && (Right[i] - Left[i] > 0.001 * Left[i] || Left[i] == 0 || Right[i] == 0))
                {
                    redo_particle = 1;
                }
                ...
                if (redo_particle)
                {
                    ...
                    if (P[i].DMB_NumNgb < desnumngb - desnumngbdev)
                    {
                        Left[i] = DMAX(P[i].DMB_Hsml, Left[i]);
                    }
                    if (P[i].DMB_NumNgb > desnumngb + desnumngbdev)
                    {
                        if (Right[i] > 0)
                        {
                            Right[i] = DMIN(Right[i], P[i].DMB_Hsml);
                        }
                        else
                        {
                            Right[i] = P[i].DMB_Hsml;
                        }
                    }
                    ...
                    if (Right[i] > 0 && Left[i] > 0)
                    {
                        ...
                        {
                            if (P[i].DMB_Hsml > Right[i])
                                P[i].DMB_Hsml = Right[i];
                            if (P[i].DMB_Hsml < Left[i])
                                P[i].DMB_Hsml = Left[i];
                    ...
                    else
                    {
                        ...
                        {
                            char buf[1000];
                            sprintf(buf, "DMB: Right[i] == 0 && Left[i] == 0 && P[i].DMB_Hsml=%g\n", P[i].DMB_Hsml);
                            terminate(buf);
                        }
                        double fac;
                        if (Right[i] == 0 && Left[i] > 0)
                        {
                            if (P[i].DMB_NumNgb > 1)
                            {
                                fac = log(desnumngb / P[i].DMB_NumNgb) / NUMDIMS;
                            }
                            else
                            {
                                fac = 1.4;
                            }
                            if ((P[i].DMB_NumNgb < 2 * desnumngb) && (P[i].DMB_NumNgb > 0.1 * desnumngb))
                            {
                                P[i].DMB_Hsml *= exp(fac);
                            }
                            else
                            {
                                P[i].DMB_Hsml *= 1.26;
        ...
        if (ntot > 0)
        {
            ...
            {
                if (iter > 10)
                    printf("DMB: ngb iteration %d: need to repeat for %d%09d particles.\n", iter, (int)(ntot / 1000000000), (int)(ntot % 1000000000));
            }
            if (iter > MAXITER)
            {
                printf("DMB: failed to converge in neighbour iteration in dmb_calc()\n");
                fflush(stdout);
                endrun(1155);
            }
        }
    ...
    for (i = FirstActiveParticle; i >= 0; i = NextActiveParticle[i])
    {
        if (dmb_isactive(i))
        {
            ...
            else
            {
                ...
                if (P[i].AGS_VelDisp > 0)
                    dt_veldisp_ratio = P[i].DMB_DtInternalEnergy / P[i].AGS_VelDisp;
